...
Data fetching is owned by an independent **data tier** (`data_service/`, FastAPI, in-memory TTL cache) — both the frontend (display) and the backend (computation: indicators, scoring, and portfolio optimization built in this repo; backtesting contributed from a teammate's branch) fetch raw data from it instead of hitting external APIs directly. See [data_service/README.md](data_service/README.md) for the API contract.
